  ...
  useEffect(() => {
    if (placeId) {
      const fetchPlace = async () => {
        const { data } = await axios.get(`/places/singlePlace/${placeId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (photos) {
      try {
        ...
        await axios.put(`/places/editplace/${placeId}`, postData);
        toast.success("Places Update successfully!");
        navigate("/");
      } catch (error) {
        toast.error("Something Updating went wrong");
        console.log(error);
      }
    }
